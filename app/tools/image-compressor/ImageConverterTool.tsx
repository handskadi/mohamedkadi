...
export default function ImageConverterTool() {
  ...
  const updateQuality = async (id: string) => {
    ...
    setImages(prev =>
      prev.map(i => (i.id === id ? { ...i, isUpdating: true, qualityOverride: i.tempQuality } : i))
    );
  ...
  const handleSliderChange = (id: string, value: number) => {
    setImages(prev => prev.map(img => (img.id === id ? { ...img, tempQuality: value } : img)));
  };
  ...
  const toggleCompare = (id: string) => {
    setImages(prev =>
      prev.map(img => (img.id === id ? { ...img, showCompare: !img.showCompare } : img))
    );
  };
