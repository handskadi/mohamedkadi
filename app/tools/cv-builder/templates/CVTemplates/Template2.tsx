    ...
    return (
        <div style={{ display: "flex", fontFamily: "system-ui", fontSize: 15, color: "#111827" }}>
            ...
            <div style={{
                ...
                {personal.photo && (
                    <img src={personal.photo} alt="profile" style={{ width: 100, height: 100, borderRadius: "50%", marginBottom: 16 }} />
                )}
                ...
                {show("skills", skills) && (
                    <SidebarSection title="Skills">
                        {skills.map((s, i) => <li key={i}>• {s.name}{s.level && ` — ${labelLevel(s.level)}`}</li>)}
                    </SidebarSection>
                )}

                {show("languages", languages) && (
                    <SidebarSection title="Languages">
                        {languages.map((l, i) => <li key={i}>• {l.name}{l.level && ` — ${labelLevel(l.level)}`}</li>)}
                    </SidebarSection>
                )}
            ...
            <div style={{ width: "65%", padding: 32 }}>
                ...
                {show("education", education) && (
                    <MainSection title="Education">
                        {education.map((edu, idx) => (
                            <div key={idx} style={{ marginBottom: 12 }}>
                                <div style={{ fontWeight: 600 }}>{edu.degree} at {edu.school}{edu.city && ` — ${edu.city}`}</div>
                                <div style={{ fontSize: 13, color: "#6B7280" }}>{edu.startDate} – {edu.endDate}</div>
                                <p>{edu.description}</p>
                            </div>
                        ))}
                    </MainSection>
                ...
                {show("courses", courses) && (
                    <MainSection title="Courses">
                        {courses.map((c, idx) => (
                            <div key={idx} style={{ marginBottom: 12 }}>
                                <div style={{ fontWeight: 600 }}>{c.name} at {c.institution}</div>
                                <div style={{ fontSize: 13, color: "#6B7280" }}>{c.start} – {c.end}</div>
                                <p>{c.description}</p>
                            </div>
                        ))}
                    </MainSection>
                ...
                {show("references", references) && (
                    <MainSection title="References">
                        {references.map((ref, idx) => (
                            <div key={idx} style={{ marginBottom: 12 }}>
                                <div style={{ fontWeight: 600 }}>{ref.person} — {ref.company}</div>
                                <div style={{ fontSize: 13, color: "#6B7280" }}>{ref.email} · {ref.phone}</div>
                            </div>
                        ))}
                    </MainSection>
                )}
                ...
                {show("custom", customSections) && (
                    <MainSection title="Additional Info">
                        <ul>{customSections.map((c, i) => <li key={i}>• {c}</li>)}</ul>
                    </MainSection>
                )}
            </div>
        </div>
