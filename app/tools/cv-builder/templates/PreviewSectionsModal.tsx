...
export default function PreviewSectionsModal({
  ...
  const { togglePreviewSection, hiddenSections } = useCVStore();
    ...
    <Dialog open={open} onClose={onClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="w-full max-w-md rounded bg-white p-6 shadow-xl space-y-6">
          <Dialog.Title className="text-lg font-semibold">Show / Hide CV Sections</Dialog.Title>
            ...
            {allSections.map(key => (
              <div key={key} className="flex justify-between items-center border-b pb-2">
                ...
                <Switch
                  ...
                  className={`${
                    !hiddenSections.includes(key) ? "bg-green-500" : "bg-gray-300"
                  } relative inline-flex h-6 w-11 items-center rounded-full`}
                >
                  <span className="sr-only">Toggle section</span>
                  <span
                    className={`${
                      !hiddenSections.includes(key) ? "translate-x-6" : "translate-x-1"
