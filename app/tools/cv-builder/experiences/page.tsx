...
export default function ExperiencesPage() {
    const {
        ...
        interests, addInterest, updateInterest, removeInterest,
        references, addReference, updateReference, removeReference,
        languages, addLanguage, updateLanguage, removeLanguage,
        courses, addCourse, updateCourse, removeCourse,
        publications, addPublication, updatePublication, removePublication,
        ...
        visibleSections, toggleSection, reorderSections
    } = useCVStore();
...
function SortableItem({
    ...
    const {
        ...
        skills, updateSkill, removeSkill, addSkill,
        interests, updateInterest, removeInterest, addInterest,
        references, updateReference, removeReference, addReference,
        languages, updateLanguage, removeLanguage, addLanguage,
        courses, updateCourse, removeCourse, addCourse,
        publications, updatePublication, removePublication, addPublication,
        achievements, updateAchievement, removeAchievement, addAchievement,
        customSections, updateCustomSection, removeCustomSection, addCustomSection
    } = useCVStore();
                        ...
                        {sectionKey === "experience" &&
                            experience.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeExperience(i)}>
                                    <Input label="Job title" value={item.title} onChange={(v) => updateExperience(i, "title", v)} />
                                    <Input label="City" value={item.city} onChange={(v) => updateExperience(i, "city", v)} />
                                    <Input label="Employer" value={item.employer} onChange={(v) => updateExperience(i, "employer", v)} />
                                    <Input label="Start date" value={item.startDate} onChange={(v) => updateExperience(i, "startDate", v)} />
                                    <Input label="End date" value={item.endDate} onChange={(v) => updateExperience(i, "endDate", v)} />
                                    <TextArea label="Description" value={item.description} onChange={(v) => updateExperience(i, "description", v)} />
                                </EntryBlock>
                            ))}
                        {sectionKey === "experience" && <AddButton onClick={addExperience} label="Add experience" />}

                        {sectionKey === "education" &&
                            education.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeEducation(i)}>
                                    <Input label="Degree" value={item.degree} onChange={(v) => updateEducation(i, "degree", v)} />
                                    <Input label="City" value={item.city} onChange={(v) => updateEducation(i, "city", v)} />
                                    <Input label="School" value={item.school} onChange={(v) => updateEducation(i, "school", v)} />
                                    <Input label="Start" value={item.startDate} onChange={(v) => updateEducation(i, "startDate", v)} />
                                    <Input label="End" value={item.endDate} onChange={(v) => updateEducation(i, "endDate", v)} />
                                    <TextArea label="Description" value={item.description} onChange={(v) => updateEducation(i, "description", v)} />
                                </EntryBlock>
                            ))}
                        {sectionKey === "education" && <AddButton onClick={addEducation} label="Add education" />}

                        {sectionKey === "skills" &&
                            skills.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeSkill(i)}>
                                    <Input label="Skill" value={item.name} onChange={(v) => updateSkill(i, "name", v)} />
                                    <select
                        ...
                        {sectionKey === "interests" &&
                            interests.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeInterest(i)}>
                                    <Input label="Interest" value={item.name} onChange={(v) => updateInterest(i, "name", v)} />
                                </EntryBlock>
                            ))}
                        ...
                        {sectionKey === "references" &&
                            references.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeReference(i)}>
                                    <Input label="Company" value={item.company} onChange={(v) => updateReference(i, "company", v)} />
                                    <Input label="Person" value={item.person} onChange={(v) => updateReference(i, "person", v)} />
                                    <Input label="Phone" value={item.phone} onChange={(v) => updateReference(i, "phone", v)} />
                                    <Input label="Email" value={item.email} onChange={(v) => updateReference(i, "email", v)} />
                                </EntryBlock>
                            ))}
                        {sectionKey === "references" && <AddButton onClick={addReference} label="Add reference" />}

                        {sectionKey === "languages" &&
                            languages.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeLanguage(i)}>
                                    <Input label="Language" value={item.name} onChange={(v) => updateLanguage(i, "name", v)} />
                                    <select
                                        // className="form-select-custom"
                                        className="w-[95%] border border-gray-300 rounded px-3 py-2 text-sm"
                        ...
                        {sectionKey === "courses" &&
                            courses.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeCourse(i)}>
                                    <Input label="Course" value={item.name} onChange={(v) => updateCourse(i, "name", v)} />
                                    <Input label="Institution" value={item.institution} onChange={(v) => updateCourse(i, "institution", v)} />
                                    <Input label="Start" value={item.start} onChange={(v) => updateCourse(i, "start", v)} />
                                    <Input label="End" value={item.end} onChange={(v) => updateCourse(i, "end", v)} />
                                    <TextArea label="Description" value={item.description} onChange={(v) => updateCourse(i, "description", v)} />
                                </EntryBlock>
                            ))}
                        ...
                        {sectionKey === "publications" &&
                            publications.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removePublication(i)}>
                                    <TextArea label="Description" value={item} onChange={(v) => updatePublication(i, v)} />
                                </EntryBlock>
                            ))}
                        {sectionKey === "publications" && <AddButton onClick={addPublication} label="Add publication" />}

                        {sectionKey === "achievements" &&
                            achievements.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeAchievement(i)}>
                                    <TextArea label="Description" value={item} onChange={(v) => updateAchievement(i, v)} />
                                </EntryBlock>
                            ))}
                        {sectionKey === "achievements" && <AddButton onClick={addAchievement} label="Add achievement" />}

                        {sectionKey === "custom" &&
                            customSections.map((item, i) => (
                                <EntryBlock key={i} onRemove={() => removeCustomSection(i)}>
                                    <TextArea label="Description" value={item} onChange={(v) => updateCustomSection(i, v)} />
                                </EntryBlock>
                            ))}
                        {sectionKey === "custom" && <AddButton onClick={addCustomSection} label="Add custom section" />}
                    </div>
                )}
            </div>
        </div>
...
const Input = ({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) => (
    <input className="w-full border border-gray-300 rounded px-3 py-2 text-sm" placeholder={label} value={value} onChange={(e) => onChange(e.target.value)} />
);

const TextArea = ({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) => (
    <textarea className="w-full border border-gray-300 rounded px-3 py-2 text-sm" placeholder={label} value={value} onChange={(e) => onChange(e.target.value)} />
);

const AddButton = ({ onClick, label }: { onClick: () => void; label: string }) => (
    <button onClick={onClick} className="text-sm text-indigo-600 hover:underline flex items-center gap-1">
        <Plus size={16} /> {label}
    </button>
);
...
const StepCircle = ({ step, label, done = false, active = false }: any) => (
    <div className={`flex items-center gap-1 ${done ? "opacity-100" : active ? "opacity-90" : "opacity-60"}`}>
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold ${active ? "bg-white text-green-600" : "border-2 border-white text-white"}`}>
            {step}
        </div>
        <span>{label}</span>
    </div>
